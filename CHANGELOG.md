...
**单课生成稳定：构思辅助、句型/词汇工作台与配图尺寸对齐。**

- **文本内容构思**：`POST /api/learning/content-brief-ideas` + 前端勾选填入（L1–L4 共用）。
- **句型**：可选「指定句型」；Level 3/4 系统提示去掉强制情绪反应句。
- **词汇候选**：headword 单数规范化；Level 4 候选排除本级别他课定表与 **Level 3 全部已定表词**。
- **即梦生图**：输出宽高对齐火山推荐档（含单边 ≥1024 的 1K 档）。

若使用 Git：`git tag -a v5.4.0 -m "5.4.0 单课生成稳定"`。

### 版本号位置

- 根目录 `package.json`、`server/package.json`、`web/package.json`：**5.4.0**
...
- **生成与课文**：表单 + 初稿 / 精修 / 定稿与朗读预览（原瀑布流主流程）。
- **绘本配图**：配图准备 + 生图（仅本 Tab 挂载；需绘本级别且已有定稿）。
- **句型与词汇**：句型分析 + 词汇候选 + 定表（仅本 Tab 挂载；L1–L4 且已有定稿）。

换阅读级别时回到「生成与课文」；换课次保留当前 Tab。目的：减少单次 React 渲染节点、加快 Tab 与滚动体感。

若使用 Git：`git tag -a v5.2.0 -m "5.2.0 课文/图片/知识点三模块"`。

### 版本号位置

- 根目录 `package.json`、`server/package.json`、`web/package.json`：**5.2.0**
- `web/src/appVersion.ts`：与上同步
